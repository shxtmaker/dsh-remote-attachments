...
# 工作区根：包含 schemas/remote-attachments/v1 的最近祖先目录（兼容独立仓库与 launcher 仓库内两种布局）。
repo_root="$plugin_root"
while [[ "$repo_root" != "/" && ! -d "$repo_root/schemas/remote-attachments/v1" ]]; do
...
tarball="$plugin_root/pack/$(basename "$tarball_rel")"
